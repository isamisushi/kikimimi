...
impl FromRequestParts<AppState> for WebSessionContext {
    ...
    async fn from_request_parts(parts: &mut Parts, state: &AppState) -> Result<Self, Self::Rejection> {
        ...
        let row: Option<(Uuid, Uuid, String, bool, DateTime<Utc>)> = sqlx::query_as(
            "SELECT s.account_id, s.org_id, a.email, s.revoked, s.expires_at \
        ...
        let (account_id, org_id, email, revoked, expires_at) =
        ...
        Ok(WebSessionContext { account_id, org_id, email })
    }
}
...
/// `GET /web/me` → `{email, org_id}` | 401. The 401 case never reaches this
/// handler body at all — [`WebSessionContext`]'s `FromRequestParts`
/// rejection *is* the 401 (same pattern as `auth::AuthContext`).
pub async fn me(session: WebSessionContext) -> Json<Value> {
    Json(json!({ "email": session.email, "org_id": session.org_id }))
}
