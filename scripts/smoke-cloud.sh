#!/usr/bin/env bash
# guru Stage 0 cloud smoke test (architecture.md §6, §8, §12).
#
# End-to-end, two-host check against a real guru-cloud server + a real
# (throwaway) Postgres database in the dev Docker container:
#
#   1. cargo build --release (workspace)
#   2. fresh test database + guru-cloud on 127.0.0.1:8790 (GURU_DEV_AUTOAPPROVE=1)
#   3. host A: guru login (autoapprove) -> guru agent -> 3 hook fixtures
#      (SessionStart, PreToolUse/PostToolUse mcp__github__get_issue) -> guru
#      flush -> stop agent
#   4. host B: same, different GURU_DIR/XDG_RUNTIME_DIR/OTLP port, a Bash
...
#      unchanged
#   6. `guru query --cloud tools` (host A) shows both hosts' tools; `guru
#      export`'s parquet row count matches psql (via the duckdb CLI)
#   7. RLS negative check: a second guru-cloud process with
#      GURU_DEV_EMAIL=other@local, a fresh login against it -> its
...
TEST_DB="guru_smoke_$(date +%s)_$$"

WORKDIR=/tmp/guru-e2e
GURU_DIR_A="$WORKDIR/a"
RT_A="$WORKDIR/a-rt"
GURU_DIR_B="$WORKDIR/b"
RT_B="$WORKDIR/b-rt"
GURU_DIR_C="$WORKDIR/c"
RT_C="$WORKDIR/c-rt"
...
CLOUD_BIN="$REPO_ROOT/target/release/guru-cloud"

rm -rf "$WORKDIR"
mkdir -p "$GURU_DIR_A" "$RT_A" "$GURU_DIR_B" "$RT_B" "$GURU_DIR_C" "$RT_C"
...
wait_daemon_ready() {
  ...
  local i=0
  while true; do
    if env GURU_DIR="$dir" XDG_RUNTIME_DIR="$rt" "$BIN" flush >/dev/null 2>&1; then
      return 0
    fi
    if ! kill -0 "$pid" 2>/dev/null; then
      echo "smoke-cloud.sh: agent (pid $pid, GURU_DIR=$dir) died before becoming ready" >&2
      return 1
    fi
    i=$((i + 1))
    if [[ "$i" -ge 100 ]]; then
      echo "smoke-cloud.sh: agent (pid $pid, GURU_DIR=$dir) never became reachable on its control socket" >&2
      return 1
    fi
    sleep 0.1
  done
...
# is asynchronous: `guru flush` only guarantees the daemon *received* the
# request, not that it has finished POSTing to guru-cloud yet).
wait_for_row_count() {
...
feed_hook() {
  ...
  local out status
  set +e
  out=$(printf '%s' "$payload" | env GURU_DIR="$dir" XDG_RUNTIME_DIR="$rt" "$BIN" hook "$event")
  status=$?
  set -e
  if [[ "$status" -ne 0 ]]; then
    echo "smoke-cloud.sh: guru hook $event (GURU_DIR=$dir) exited $status (must always be 0)" >&2
    exit 1
  fi
  if [[ -n "$out" ]]; then
    echo "smoke-cloud.sh: guru hook $event (GURU_DIR=$dir) printed output on success: $out" >&2
    exit 1
  fi
...
# --- 2. fresh test db + guru-cloud ------------------------------------------

echo "==> [2/8] creating test database $TEST_DB and starting guru-cloud on $CLOUD_BIND"
psql_admin -c "CREATE DATABASE \"${TEST_DB}\";"
DATABASE_URL="postgres://${PG_SUPERUSER}:${PGPASSWORD}@${PG_HOST}:${PG_PORT}/${TEST_DB}"

env BIND_ADDR="$CLOUD_BIND" DATABASE_URL="$DATABASE_URL" GURU_DEV_AUTOAPPROVE=1 \
  "$CLOUD_BIN" >"$WORKDIR/cloud.log" 2>&1 &
CLOUD_PID=$!
wait_http_ok "$CLOUD_URL/healthz" || { cat "$WORKDIR/cloud.log" >&2; exit 1; }
echo "==> guru-cloud is up (pid $CLOUD_PID, db $TEST_DB)"

# --- 3. host A ---------------------------------------------------------------

echo "==> [3/8] host A: login, agent, 3 hook fixtures (SessionStart + PreToolUse/PostToolUse mcp__github__get_issue), flush, stop"

env GURU_DIR="$GURU_DIR_A" XDG_RUNTIME_DIR="$RT_A" "$BIN" login --endpoint "$CLOUD_URL" --no-browser
HOST_A_ID=$(cat "$GURU_DIR_A/host_id")
TOKEN_A=$(cfg_field "$GURU_DIR_A/config.json" token)
ORG_A=$(cfg_field "$GURU_DIR_A/config.json" org_id)
echo "==> host A logged in: host_id=$HOST_A_ID org_id=$ORG_A"

env GURU_DIR="$GURU_DIR_A" XDG_RUNTIME_DIR="$RT_A" GURU_OTLP_PORT="$OTLP_PORT_A" \
  "$BIN" agent --foreground >"$WORKDIR/agent-a.log" 2>&1 &
AGENT_A_PID=$!
wait_daemon_ready "$GURU_DIR_A" "$RT_A" "$AGENT_A_PID" || { cat "$WORKDIR/agent-a.log" >&2; exit 1; }
echo "==> host A agent is up (pid $AGENT_A_PID)"

A_SESSION_START=$(cat <<'JSON'
{
  "session_id": "sess-e2e-a",
  "transcript_path": "/tmp/guru-e2e/a/sess-e2e-a.jsonl",
  "cwd": "/tmp/guru-e2e/a",
...
A_PRETOOLUSE_MCP=$(cat <<'JSON'
{
  ...
  "tool_input": { "owner": "isamisushi", "repo": "guru", "issue_number": 42 },
...
feed_hook "$GURU_DIR_A" "$RT_A" "SessionStart" "$A_SESSION_START"
feed_hook "$GURU_DIR_A" "$RT_A" "PreToolUse" "$A_PRETOOLUSE_MCP"
feed_hook "$GURU_DIR_A" "$RT_A" "PostToolUse" "$A_POSTTOOLUSE_MCP"

env GURU_DIR="$GURU_DIR_A" XDG_RUNTIME_DIR="$RT_A" "$BIN" flush
...
env GURU_DIR="$GURU_DIR_B" XDG_RUNTIME_DIR="$RT_B" "$BIN" login --endpoint "$CLOUD_URL" --no-browser
HOST_B_ID=$(cat "$GURU_DIR_B/host_id")
TOKEN_B=$(cfg_field "$GURU_DIR_B/config.json" token)
ORG_B=$(cfg_field "$GURU_DIR_B/config.json" org_id)
...
env GURU_DIR="$GURU_DIR_B" XDG_RUNTIME_DIR="$RT_B" GURU_OTLP_PORT="$OTLP_PORT_B" \
  "$BIN" agent --foreground >"$WORKDIR/agent-b.log" 2>&1 &
AGENT_B_PID=$!
wait_daemon_ready "$GURU_DIR_B" "$RT_B" "$AGENT_B_PID" || { cat "$WORKDIR/agent-b.log" >&2; exit 1; }
echo "==> host B agent is up (pid $AGENT_B_PID)"

B_SESSION_START=$(cat <<'JSON'
{
  "session_id": "sess-e2e-b",
  "transcript_path": "/tmp/guru-e2e/b/sess-e2e-b.jsonl",
  "cwd": "/tmp/guru-e2e/b",
  "hook_event_name": "SessionStart",
  "source": "startup",
  "model": "claude-opus-4-6-20260805"
...
feed_hook "$GURU_DIR_B" "$RT_B" "SessionStart" "$B_SESSION_START"
feed_hook "$GURU_DIR_B" "$RT_B" "PreToolUse" "$B_PRETOOLUSE_BASH"
feed_hook "$GURU_DIR_B" "$RT_B" "PostToolUse" "$B_POSTTOOLUSE_BASH"

env GURU_DIR="$GURU_DIR_B" XDG_RUNTIME_DIR="$RT_B" "$BIN" flush
...
printf '{"schema":"guru.v1","events":%s}' "$EVENTS_JSON" >"$RESEND_JSON"
...
# --- 6. guru query --cloud + guru export ------------------------------------

echo "==> [6/8] guru query --cloud tools (host A) + guru export vs psql (duckdb)"

TOOLS_OUT=$(env GURU_DIR="$GURU_DIR_A" XDG_RUNTIME_DIR="$RT_A" "$BIN" query --cloud tools)
echo "$TOOLS_OUT"
...
env GURU_DIR="$GURU_DIR_A" XDG_RUNTIME_DIR="$RT_A" "$BIN" export -o "$EXPORT_PATH"
[[ -s "$EXPORT_PATH" ]] || fail "guru export did not write a non-empty file at $EXPORT_PATH"
...
echo "==> [7/8] RLS negative check: second org (GURU_DEV_EMAIL=other@local) sees 0 rows"

env BIND_ADDR="$CLOUD2_BIND" DATABASE_URL="$DATABASE_URL" GURU_DEV_AUTOAPPROVE=1 GURU_DEV_EMAIL="other@local" \
  "$CLOUD_BIN" >"$WORKDIR/cloud2.log" 2>&1 &
CLOUD2_PID=$!
wait_http_ok "$CLOUD2_URL/healthz" || { cat "$WORKDIR/cloud2.log" >&2; exit 1; }

env GURU_DIR="$GURU_DIR_C" XDG_RUNTIME_DIR="$RT_C" "$BIN" login --endpoint "$CLOUD2_URL" --no-browser
TOKEN_C=$(cfg_field "$GURU_DIR_C/config.json" token)
ORG_C=$(cfg_field "$GURU_DIR_C/config.json" org_id)
EMAIL_C=$(cfg_field "$GURU_DIR_C/config.json" email)
