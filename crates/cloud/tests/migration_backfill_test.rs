...
const MIGRATIONS_0001_TO_0006: &[(&str, &str)] = &[
    ...
    ("0002_app_role", include_str!("../migrations/0002_app_role.sql")),
...
async fn apply_legacy_schema(pool: &PgPool) {
    ...
    for (name, sql) in MIGRATIONS_0001_TO_0006 {
        let rendered = sql.replace(
            "{{APP_PASSWORD}}",
            &format!("$kikimimi_app_pw${}$kikimimi_app_pw$", support::TEST_APP_DB_PASSWORD),
        );
...
async fn backfill_gives_a_pre_existing_account_a_kind_slug_and_owner_membership() {
    ...
    let account_id: (Uuid,) = sqlx::query_as("INSERT INTO accounts (email) VALUES ($1) RETURNING id")
    ...
    let pre_has_memberships: (bool,) =
        sqlx::query_as("SELECT to_regclass('memberships') IS NOT NULL").fetch_one(&pool).await.unwrap();
    assert!(!pre_has_memberships.0, "memberships must not exist before 0007 runs");
    ...
    let applied: Vec<(String,)> = sqlx::query_as("SELECT name FROM _migrations ORDER BY name")
        ...
        .await
        .unwrap();
    assert_eq!(applied.last().unwrap().0, "0007_account_model");
    assert_eq!(applied.len(), 7);

    // org_members is gone, memberships has the same row plus created_at.
    let post_has_org_members: (bool,) =
        sqlx::query_as("SELECT to_regclass('org_members') IS NOT NULL").fetch_one(&pool).await.unwrap();
    assert!(!post_has_org_members.0, "org_members must be renamed away, not left behind");
    ...
    assert_eq!(membership.0, "owner", "backfill: every existing account keeps its owner membership");

    // orgs.kind/slug backfilled.
    let (kind, slug): (String, String) = sqlx::query_as("SELECT kind, slug FROM orgs WHERE id = $1")
        .bind(org_id.0)
        .fetch_one(&pool)
        .await
        .unwrap();
    ...
    let (invites, audit): (i64, i64) = {
        let i: (i64,) = sqlx::query_as("SELECT count(*) FROM org_invites").fetch_one(&pool).await.unwrap();
        let a: (i64,) = sqlx::query_as("SELECT count(*) FROM audit_log").fetch_one(&pool).await.unwrap();
        (i.0, a.0)
    };
    assert_eq!(invites, 0);
    assert_eq!(audit, 0);
