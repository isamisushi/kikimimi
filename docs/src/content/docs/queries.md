...
| `subagents` | per `agent_id`: type, parent `turn_id`, start, duration (the `SubagentStop` hook's, else first-to-last event), events, tool calls, failures, API requests, `tokens_est` (NULL when nothing carried usage — never 0), the distinct tools it used, and `models` / `efforts` / `model_source` — which model and effort the subagent actually ran on: from the agent's own rows (`agent`; transcript `api.request`s carry `agent_id`), else attributed from the session's OTel `api.request` rows of the same `agent_type` inside the agent's first-to-last event window ±5 s (`otel_window`; a live session has only hooks, which carry `agent_id` but no model, and OTel, which carries `agent.name` + model + effort but no `agent_id` — parallel subagents of one type share the value), NULL when neither |
