...
/// `/web/q/patterns` (KKM-11): the org-wide struggle ranking. Visible to
/// every role — architecture.md §6.1 gives `member` "org の MCP/ツール
/// ランキング" explicitly; it is an aggregate over patterns and subjects,
/// never over people (§11), so there is nothing to scope away and no
/// audit row (unlike the per-session drilldown below).
pub async fn patterns(
    ...
    let mut tx = state.pools.org_scoped_tx(session.org_id).await?;
    let stmt = (&mut *tx)
        .prepare(SqlStr::from_static(PATTERNS_SQL))
        .await
        .map_err(anyhow::Error::from)?;
    let columns: Vec<String> = stmt
        .columns()
        ...
        .map(|c| c.name().to_string())
        .collect();
    let pg_rows: Vec<PgRow> = sqlx::query(PATTERNS_SQL)
        .bind(&from_dt)
        .fetch_all(&mut *tx)
        .await
        .map_err(anyhow::Error::from)?;
...
/// `/web/q/pattern-hits` (KKM-11): the sessions behind one ranking row.
/// Same role rule and audit as [`sessions`]: in a `team` org a role below
/// `admin` only sees their own sessions ([`PATTERN_HITS_SQL_SELF`]), and an
/// admin/owner drilldown writes one `audit_log` row (`pattern_drilldown`).
pub async fn pattern_hits(
    ...
    if is_team && is_admin_plus {
        sqlx::query("INSERT INTO audit_log (actor, org_id, action, target) VALUES ($1, $2, 'pattern_drilldown', $3)")
            .bind(session.account_id)
            .bind(session.org_id)
            .bind(format!("{}:{}", q.pattern_id, q.subject))
            .execute(&state.pools.superuser)
            .await
            .map_err(anyhow::Error::from)?;
    }
