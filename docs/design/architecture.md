...
**二階建て (2026-09-07 決定、LiteLLM と同じ構造)**: ルートは Apache-2.0 (agent デーモン、CLI、アダプタ、`kikimimi.v1` スキーマ、検知 SQL、ローカル web、docs — ユーザーがインストールするものすべて)。`crates/cloud/` (ホスト型 kikimimi cloud) のみ FSL-1.1-Apache-2.0 (自社利用は商用含め自由、競合ホスト型サービスとしての提供のみ不可、各リリースは 2 年後に Apache-2.0 へ自動転換)。§15-13「個人は無料 + OSS で獲得、組織課金は cloud」に合わせた。他社が OSS の agent を使って独自 cloud を作ることは許容し、そこは製品力で競う。ホスティング/課金の方針は別途。
