#!/usr/bin/env bash
# guru Stage 0 smoke test (architecture.md §4, §12).
#
# Builds the release binary, runs a fully isolated `guru agent`, feeds it a
# bypass-shaped sequence of realistic Claude Code hook payloads plus one OTLP
# logs request, flushes to Parquet, and checks that `guru query tools` /
# `guru query bypass` can see what happened via the duckdb CLI.
set -euo pipefail
...
if ! command -v duckdb >/dev/null 2>&1; then
  echo "smoke.sh: the 'duckdb' CLI is required (guru query shells out to it) but was not found in PATH" >&2
  exit 1
fi
...
echo "==> starting guru agent --foreground in background"
...
# The last two events are exactly the shape `guru query bypass` looks for:
...
feed_hook() {
  local event="$1"
  local payload="$2"
  # guru hook must always exit 0 and print nothing on success; check both here
  ...
  set -e
  if [[ "$status" -ne 0 ]]; then
    echo "smoke.sh: guru hook $event exited $status (must always be 0)" >&2
    exit 1
  fi
  if [[ -n "$out" ]]; then
    echo "smoke.sh: guru hook $event printed output on success: $out" >&2
    exit 1
  fi
...
SESSION_START=$(cat <<'JSON'
{
  ...
  "transcript_path": "/tmp/guru-smoke/sess-smoke-1.jsonl",
  "cwd": "/tmp/guru-smoke",
...
POSTTOOLUSE_MCP_OK=$(cat <<'JSON'
{
  ...
  "tool_input": { "owner": "isamisushi", "repo": "guru", "issue_number": 1 },
...
POSTTOOLUSE_MCP_FAIL=$(cat <<'JSON'
{
  ...
  "tool_name": "mcp__github__get_issue",
  "tool_input": { "owner": "isamisushi", "repo": "guru", "issue_number": 999999 },
...
PRETOOLUSE_BASH_CURL=$(cat <<'JSON'
{
  ...
  "tool_name": "Bash",
  "tool_input": {
    "command": "curl -s https://api.github.com/repos/isamisushi/guru/issues/999999",
...
curl -sS -o /dev/null -w '%{http_code}\n' \
  -X POST "http://127.0.0.1:${GURU_OTLP_PORT}/v1/logs" \
...
if [[ "$FLUSH_OUT" != *"true"* ]]; then
  echo "smoke.sh: guru flush was not acked by the daemon" >&2
  exit 1
fi
sleep 0.5

PARQUET_COUNT=$(find "$GURU_DIR/data/events" -mindepth 2 -maxdepth 2 -type f -name '*.parquet' 2>/dev/null | wc -l | tr -d ' ')
echo "==> parquet files under dt=*/: $PARQUET_COUNT"
if [[ "$PARQUET_COUNT" -lt 1 ]]; then
  echo "smoke.sh: expected at least one dt=*/*.parquet file under $GURU_DIR/data/events" >&2
  exit 1
fi

# --- query -----------------------------------------------------------

echo "==> guru query tools"
TOOLS_OUT=$("$BIN" query tools)
echo "$TOOLS_OUT"
if ! grep -q "mcp__github__get_issue" <<<"$TOOLS_OUT"; then
  echo "smoke.sh: 'guru query tools' output did not mention mcp__github__get_issue" >&2
  exit 1
fi
if ! grep -q "Bash" <<<"$TOOLS_OUT"; then
  echo "smoke.sh: 'guru query tools' output did not mention Bash" >&2
  exit 1
fi

echo "==> guru query bypass"
BYPASS_OUT=$("$BIN" query bypass --show-sql)
echo "$BYPASS_OUT"
if ! grep -q "github" <<<"$BYPASS_OUT"; then
  echo "smoke.sh: 'guru query bypass' output did not mention the github mcp_server" >&2
  exit 1
fi
if ! grep -q "Bash" <<<"$BYPASS_OUT"; then
  echo "smoke.sh: 'guru query bypass' output did not mention the following Bash tool_call" >&2
  exit 1
fi
